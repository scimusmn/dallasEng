...
void drawHatching(double x, double y, double w, double h, double hatchSpace, double hatchWidth)
{
	if(cfg().defaultColor) {
		...
		for(unsigned int i=0; i<w/(hatchSpace+hatchWidth); i++){
			ofRect(x+(hatchSpace+hatchWidth)*i,y,hatchWidth,h);
		}
		for(unsigned int i=0; i<h/(hatchSpace+hatchWidth); i++){
			ofRect(x,y+(hatchSpace+hatchWidth)*i,w,hatchWidth);
		}
	}
	/*hatchSpace=3;
	hatchWidth=6;

	for(unsigned int j=0; j<h/(hatchSpace+hatchWidth); j++){
		for(unsigned int i=0; i<w/(hatchSpace+hatchWidth); i++){
			ofCircle(x+(hatchSpace+hatchWidth)*i,y+(hatchSpace+hatchWidth)*j,hatchWidth);
		}
	}*/
}
